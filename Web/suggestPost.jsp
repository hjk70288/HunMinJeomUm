   ...
   <!-- Post-->
    <%
     ...
     <div style="margin-top:25px" class="col-lg-9">
     	<h2 class="text-black mt-0">건의 게시판</h2>
     	<br>
            ...
            <c:set var="writer" value="<%=vo.getId()%>"/>
               <c:choose>
               	...
               	<c:when test="${sessionId eq writer}">
                        	<button class="btn  btn-dark float-right" onclick="location.href='PostServlet?cmd=suggest_delete&num=<%=vo.getPostNum()%>&date=<%=vo.getDate() %>'">삭제</button>
                        	<button class="btn  btn-dark float-right" onclick="location.href='PostServlet?cmd=suggest_update&num=<%=vo.getPostNum()%>&date=<%=vo.getDate() %>'">수정</button>       
               				<br><br>
               </c:when>
               	<c:when test="${sessionId eq 'admin'}">
                        	<button class="btn  btn-dark float-right" onclick="location.href='PostServlet?cmd=suggest_delete&num=<%=vo.getPostNum()%>&date=<%=vo.getDate() %>'">삭제</button>    
        ...
        <br>
        <!-- 댓글 부분 -->
        <form action="PostServlet?cmd=suggest_comment_add&id=<%=sessionId%>&num=<%=vo.getPostNum() %>" method="post">
    	<div id="comment">
        <table border="1" bordercolor="lightgray">
   		 <!-- 댓글 목록 -->   
   		 <%
     				CommentDAO commentDAO = new CommentDAO();
     				List<CommentVO> commentList = commentDAO.getSuggestCommentList(vo.getId(), vo.getPostNum());
               ...
               <c:choose>
               	...
               	<c:when test="${sessionId eq writer}">
					<td width="100" align="center">  
						<button type="button" class="btn  btn-dark" onclick="location.href='PostServlet?cmd=suggest_comment_delete&id=<%=sessionId%>&num=<%=vo.getPostNum()%>&date=<%=commentVO.getDate()%>'">삭제</button>
                	</td>
               </c:when>
               	<c:when test="${sessionId eq 'admin'}">
                	<td width="100" align="center">
                		<button type="button" class="btn  btn-dark" onclick="location.href='PostServlet?cmd=suggest_comment_delete&id=<%=commentVO.getId()%>&num=<%=vo.getPostNum()%>&date=<%=commentVO.getDate()%>'">삭제</button>
               		 </td>
               	</c:when>
               	<c:otherwise>
               		<td width="100">        
