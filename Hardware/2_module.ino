...
// 점자표시기 세팅
#include "braille.h"
int dataPin[2] = {4, 7}; // DATA 핀번호
int latchPin[2] = {5, 8}; // LATCH 핀번호
int clockPin[2] = {6, 9}; // CLOCK 핀번호
int no_module = 3; // 점자 출력기 연결 개수
braille bra(dataPin[0], latchPin[0], clockPin[0], no_module);
braille bra2(dataPin[1], latchPin[1], clockPin[1], no_module);
...
void consonant_print(int index) {
  bra.all_off();
  bra2.all_off();
  for ( int i = 0; i < 6; i++)
  {
...
void collection_print(int index) {
  bra.all_off();
  bra2.all_off();
  for ( int i = 0; i < 6; i++)
  {
